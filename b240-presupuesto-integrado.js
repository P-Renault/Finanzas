...
/* Adaptador integrado: primero consume puentes globales B2.36–B2.39,
   luego nombres de datos del núcleo recuperado. */
...
async function loadOwnBudget(){
  ...
  if(!c||!user) throw new Error('No existe una sesión Supabase activa.');
  const b=await c.from('presupuestos').select('*').eq('user_id',user).eq('periodo',period()).maybeSingle();
  if(b.error) throw b.error;
  if(!b.data){
    const ins=await c.from('presupuestos').insert({
      user_id:user,periodo:periodo(),nombre:`Presupuesto ${monthLabel()}`,estado:'activo'
    }).select().single();
    if(ins.error) throw ins.error;
    S.budget=ins.data;
  }else S.budget=b.data;

  const q=await c.from('presupuesto_partidas').select('*').eq('presupuesto_id',S.budget.id).order('tipo').order('categoria');
