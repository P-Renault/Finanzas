...
var VERSION='B232.60-RELEASE-MODULOS-REALES';
...
function isPlaceholder(s){if(!s)return false;var x=(s.textContent||'').replace(/\s+/g,' ').trim();return /^(Motor Multifuente Módulo disponible mediante su integración financiera existente\.?|Control de Jornada Módulo disponible mediante su integración operativa existente\.?)$/i.test(x)}
...
function boot(){nav();ensureReal('ingresos',incomeHtml);ensureReal('jornadas',jobsHtml);wire();footer()}
