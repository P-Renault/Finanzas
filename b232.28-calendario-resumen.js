...
async function refreshModule(id){
  try{
    ...
    if(id==='operaciones' && legacyShow){
      await Promise.resolve(legacyShow('operaciones')); normalize('operaciones'); return;
    }
    if(id==='planificacion'){
      if(window.fin216Plan) await window.fin216Plan();
      else if(legacyShow) await Promise.resolve(legacyShow('planificacion'));
      normalize('planificacion'); return;
    }
    if(id==='ingresos' && legacyShow){await Promise.resolve(legacyShow('ingresos'));normalize('ingresos');return;}
    if(id==='jornadas' && legacyShow){await Promise.resolve(legacyShow('jornadas'));normalize('jornadas');return;}
  ...
  }catch(e){console.error('[B232.27] refresh module',id,e);normalize(id);}
}

async function show(id){
  if(!normalize(id))return false;
  await refreshModule(id);
  normalize(id);
  return true;
}
window.CCFRouter={version:'232.27',show,normalize,refreshModule};
