/* B232.34 — RESUMEN INTEGRADO CONSERVADOR
   FIX 2026-09-26 · SINGLE SOURCE OF TRUTH
   - B232.34 es el propietario exclusivo del cuadro .b234-summary.
   - Usa exclusivamente window.supabaseClient.
   - Exige usuario autenticado antes de consultar.
   - El resumen consolidado se deriva de las mismas categorías renderizadas.
   - No crea clientes Supabase alternativos.
*/
(()=>{'use strict';
const VERSION='B232.34-FIX-2026.09.26';
...
/* ÚNICO cliente permitido para este módulo. */
function db(){
  const c=window.supabaseClient;
  if(c?.auth) return c;
  console.error('[B232.34] Cliente Supabase autenticado no disponible.');
  return null;
}
...
async function collect(month){
  ...
  /* Usuario verificado: no consultar con un cliente sin sesión. */
  const auth=await c.auth.getUser();
  if(auth?.error || !auth?.data?.user?.id){
    console.error('[B232.34] No hay usuario autenticado; se cancela la lectura.');
    return null;
  }

  const start=monthStart(month),end=monthEnd(month);
  const [mov,future,planned,comm,quotas,debt,banks,close,budgetLines]=await Promise.all([
  ...
  const totalIncome=income+futureIncome;

  const cats={};
  const add=(cat,real,pending,fut)=>{
    const k=String(cat||'Otros').trim()||'Otros';
    cats[k]??={real:0,pending:0,future:0};
    cats[k].real+=real;cats[k].pending+=pending;cats[k].future+=fut;
  };
...
async function render(){
  ...
  styles();
  const month=ym($('b234Month')?.value||today());
  try{
    const data=await collect(month);
    if(data)build(data,month);
  }catch(e){
    console.error('[B232.34]',e);
  }
}
...
function install(){
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('[data-tab="dashboard"]'))setTimeout(render,160);
  },true);
  setInterval(()=>{
    if(document.querySelector('[data-tab="dashboard"].active'))render();
  },1500);
  render();
}
