...
(function(){
'use strict';
if(window.__B23265_RELEASE__) return;
window.__B23262_RELEASE__=true;
var VERSION='B232.65-RELEASE-INTEGRACION-LIQUIDEZ';
...
function loadLiquidityAccounts(){
 var c=db(); if(!c||!$('b219JAccount'))return Promise.resolve([]);
 return c.from('cuentas_bancarias').select('id,nombre_banco,nombre_cuenta,saldo_actual,activa').eq('activa',true).order('nombre_banco').then(function(r){
   if(r.error){console.error('[B232.65] cuentas',r.error);$('b219JAccount').innerHTML='';return []}
   var rows=r.data||[];
   $('b219JAccount').innerHTML=rows.map(function(x){
     return '<option value="'+esc(x.id)+'">'+esc(x.nombre_banco)+' · '+esc(x.nombre_cuenta)+' · '+money(x.saldo_actual)+'</option>';
   }).join('');
   return rows;
 });
}
function syncJobLiquidity(){
 var bank=$('b219JMedium')&&$('b219JMedium').value==='cuenta_bancaria';
 if($('b219JAccountWrap'))$('b219JAccountWrap').style.display=bank?'flex':'none';
 if(bank)loadLiquidityAccounts();
}
function wire(){
 var d=today();
 if($('b219Date'))$('b219Date').value=d;
 if($('b219JDate'))$('b219JDate').value=d;
 if($('b219JMedium')&&!$('b219JMedium').dataset.b23265){
   $('b219JMedium').dataset.b23265='1';
   $('b219JMedium').addEventListener('change',syncJobLiquidity);
   syncJobLiquidity();
 }
 var fi=$('b219IncomeForm');
 if(fi&&!fi.dataset.b23260){
   fi.dataset.b23260='1';
   fi.onsubmit=async function(e){
     e.preventDefault();
     var c=db(),m=$('b219IncomeMsg');
     if(!c){m.textContent='Conecta Supabase.';return}
     var state=$('b219State').value;
     var p={
       fuente_id:Number($('b219Source').value),
       actividad:$('b219Activity').value.trim(),
       cliente:$('b219Client').value.trim()||null,
       fecha_generacion:$('b219Date').value,
       fecha_cobro:state==='cobrado'?($('b219Collection').value||d):null,
       monto_bruto:+$('b219Gross').value||0,
       costos:+$('b219Cost').value||0,
       comisiones:+$('b219Comm').value||0,
       estado_cobro:state,
       notas:$('b219Notes').value.trim()||null
     };
     if(!p.actividad||p.monto_bruto<0||p.costos<0||p.comisiones<0){m.textContent='Completa actividad y montos válidos.';return}
     m.textContent='Registrando…';
     var r=await c.from('generacion_ingresos').insert(p).select('id').single();
     if(r.error){m.textContent='No se pudo registrar: '+(r.error.message||r.error.code||'error');return}
     m.textContent='Generación registrada.';
     fi.reset();$('b219Date').value=today();loadIncome();
   }
 }
 var fj=$('b219JForm');
 if(fj&&!fj.dataset.b23265){
   fj.dataset.b23265='1';
   if($('b219JMedium'))syncJobLiquidity();
   fj.onsubmit=async function(e){
     e.preventDefault();
     var c=db(),m=$('b219JMsg'),btn=fj.querySelector('button[type="submit"]');
     if(!c){m.textContent='Conecta Supabase.';return}
     var f=await c.from('fuentes_ingreso').select('id').eq('nombre','Uber / inDrive').maybeSingle();
     if(f.error){m.textContent='No se pudo consultar la fuente: '+f.error.message;return}
     if(!f.data||!f.data.id){m.textContent='No existe la fuente Uber / inDrive en Supabase.';return}
     var net=+$('b219JNetInput').value||0,cost=+$('b219JCost').value||0,state=$('b219JState').value,date=$('b219JDate').value,ref=$('b219JRef').value.trim()||'Integración financiera de jornada';
     var medium=$('b219JMedium')?.value||'efectivo';
     var accountId=medium==='cuenta_bancaria'?Number($('b219JAccount')?.value||0):null;
     if(!date){m.textContent='La fecha de jornada es obligatoria.';return}
     if(net<0||cost<0){m.textContent='Los montos no pueden ser negativos.';return}
     if(medium==='cuenta_bancaria'&&!accountId){m.textContent='Selecciona la cuenta bancaria.';return}
     if(state==='cobrado' && net<=0){m.textContent='El neto cobrado debe ser mayor que cero.';return}
     var payload={
       p_fuente_id:Number(f.data.id),
       p_actividad:'Resultado Control de Jornada',
       p_descripcion:ref,
       p_fecha_generacion:date,
       p_monto_neto:net,
       p_costos:cost,
       p_estado_cobro:state,
       p_fecha_cobro:state==='cobrado'?date:null,
       p_medio_pago:medium,
       p_cuenta_id:accountId,
       p_notas:'Origen: Control de Jornada · B232.65'
     };
     m.textContent='Integrando resultado y actualizando liquidez…';
     if(btn){btn.disabled=true;btn.textContent='Integrando…'}
     var res=await c.rpc('integrar_jornada_liquidez_v1',payload);
     if(res.error){
       console.error('[B232.65] Jornada',res.error,payload);
       m.textContent='No se pudo integrar: '+(res.error.message||res.error.code||'error Supabase')+(res.error.code?' ['+res.error.code+']':'');
       if(btn){btn.disabled=false;btn.textContent='Integrar resultado financiero'}
       return;
     }
     m.textContent='Resultado integrado: '+money(net)+'. Liquidez actualizada.';
     fj.reset();
     $('b219JDate').value=today();
     if($('b219JMedium')){$('b219JMedium').value='efectivo';syncJobLiquidity()}
     await loadJobs();
