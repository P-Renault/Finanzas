/* B232.24 — corrección funcional de módulos, datos y calendario */
(()=>{'use strict';
...
async function renderPlan(){if(typeof window.fin216Plan==='function'){try{await window.fin216Plan();return}catch(e){console.error(e)}}const c=client();if(!c)return;const box=$('b216Content');if(!box)return;const [close,banks,inc,exp,quota]=await Promise.all([read(c.from('cierres_financieros').select('saldo_efectivo_actual').eq('activo',true).order('fecha_corte',{ascending:false}).limit(1)),read(c.from('cuentas_bancarias').select('saldo_actual').eq('activa',true)),read(c.from('ingresos_futuros').select('monto,fecha').gte('fecha',today())),read(c.from('gastos_planificados').select('monto,fecha').gte('fecha',today())),read(c.from('cuotas_deuda').select('monto,fecha_vencimiento,estado').in('estado',['pendiente','vencida']).gte('fecha_vencimiento',today()))]);const liquidity=Number(close[0]?.saldo_efectivo_actual||0)+banks.reduce((s,x)=>s+Number(x.saldo_actual||0),0),income=inc.reduce((s,x)=>s+Number(x.monto||0),0),out=exp.reduce((s,x)=>s+Number(x.monto||0),0)+quota.reduce((s,x)=>s+Number(x.monto||0),0),projected=liquidity+income-out;box.innerHTML=`<div class="card"><h2>Planificación financiera</h2><div class="b23224-kpis"><div class="b23224-kpi"><span>Liquidez real</span><strong>${money(liquidity)}</strong></div><div class="b23224-kpi"><span>Ingresos futuros</span><strong>${money(income)}</strong></div><div class="b23224-kpi"><span>Obligaciones</span><strong>${money(out)}</strong></div><div class="b23224-kpi"><span>Saldo proyectado</span><strong class="${projected>=0?'b23224-positive':'b23224-negative'}">${money(projected)}</strong></div></div></div>`}
...
// B232.29 — carga bajo demanda real. El clic del usuario es el disparador
// de la consulta; no se depende de una recarga de página ni de un timer.
function ensureModuleSection(id){
  if($(id)) return true;
  try{ if(typeof buildSections==='function') buildSections(); }catch(e){console.warn('[B232.29] buildSections',e)}
  return !!$(id);
}

function laterPlanningRefresh(){
  // Si el módulo legacy de Planificación termina de cargarse después del clic,
  // se actualiza una sola vez sin exigir F5 al usuario.
  let tries=0;
  const timer=setInterval(async()=>{
    tries++;
    if(typeof window.fin216Plan==='function'){
      clearInterval(timer);
      try{ await window.fin216Plan(); normalize('planificacion'); }catch(e){console.warn('[B232.29] fin216Plan',e)}
    }else if(tries>=15){
      clearInterval(timer);
    }
  },200);
...
async function refreshModule(id){
  try{
    ...
    if(id==='planificacion'){
      if(typeof window.fin216Plan==='function'){
        await window.fin216Plan();
      }else{
        await renderPlan();
        laterPlanningRefresh();
      }
  ...
  }catch(e){console.error('[B232.29] refresh module',id,e);normalize(id);}
}
...
window.CCFRouter={version:'232.29',show,normalize,refreshModule};
