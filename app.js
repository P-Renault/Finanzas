...
function renderCalendarDayDetails(row, key, movBy, futBy){
  ...
  <div class="day-detail-grid">
    <article class="day-detail-block income-detail"><div class="detail-title"><div><span>Ingresos</span><strong>${CLP(totalIncome)}</strong></div><span class="detail-count">${incomes.length} registro${incomes.length===1?'':'s'}</span></div>${incomes.length?incomes.map(movementRow).join(''):'<p class="muted">No hay ingresos registrados para este día.</p>'}</article>
    <article class="day-detail-block expense-detail"><div class="detail-title"><div><span>Gastos</span><strong>${CLP(totalExpense)}</strong></div><span class="detail-count">${expenses.length} registro${expenses.length===1?'':'s'}</span></div>${expenses.length?expenses.map(movementRow).join(''):'<p class="muted">No hay gastos registrados para este día.</p>'}${payments.length?`<div class="scheduled-note"><b>Pagos programados pendientes: ${CLP(totalPayments)}</b><span>${payments.length} compromiso${payments.length===1?'':'s'} para esta fecha. Se muestran aquí como referencia y no se mezclan con los gastos registrados.</span></div>`:''}</article>
  </div>
