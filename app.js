...
async function connect(){
  ...
  try{
    ...
    msg('configMsg','');
    // No bloquear el arranque de la interfaz por una tabla/RLS que falle.
    // La verificación acontece em paralelo y deja el error visible sin ocultar la app.
    ...
    // La aplicación sigue visible para poder diagnosticar el módulo que falle.
  }
}
...
function renderCalendar(mov,fut){
  ...
  $('calendarGrid').innerHTML=days.map(d=>{const key=dateKey(d),inMonth=d.getMonth()===m,r=rowMap[key];const inc=r?.inc||0,exp=r?.exp||0,pay=r?.pay||0;const items=[];for(const x of (r?.mov||[]))items.push(`<div class="day-event ${x.tipo}">${x.tipo==='ingreso'?'Ingreso':'Gasto'} · ${CLP(x.monto)}</div>`);for(const x of (r?.comms||[]))items.push(`<div class="day-event pago">Pago · ${CLP(x.monto)}</div>`);const more=items.length>3?`<div class="day-more">+${items.length-3} más</div>`:'';return `<button type="button" class="calendar-day ${inMonth?'':'outside'} ${key===t?'today-day':''} ${key===calendarSelectedDate?'selected-day':''}" data-date="${key}" onclick="selectCalendarDay('${key}')"><div class="day-head"><span class="day-number">${d.getDate()}</span>${key===t?'<span class="today-dot">HOY</span>':''}</div>${inc?`<div class="day-line income">↑ ${CLP(inc)}</div>`:''}${exp?`<div class="day-line expense">↓ ${CLP(exp)}</div>`:''}${pay?`<div class="day-line payment">● ${CLP(pay)}</div>`:''}${items.slice(0,3).join('')}${more}${inMonth&&r?`<div class="day-balance">Saldo ${CLP(r.balance)}</div>`:''}</button>`}).join('');
  renderCalendarDayDetails(rowMap[calendarSelectedDate],calendarSelectedDate,movBy,futBy);
}
window.selectCalendarDay=key=>{calendarSelectedDate=key;renderFromCache();const details=document.getElementById('calendarDayDetails');if(details)details.scrollIntoView({behavior:'smooth',block:'nearest'})};
function renderCalendarDayDetails(row,key,movBy,futBy){const box=$('calendarDayDetails');if(!box)return;const selected=parseDate(key);const dateLabel=selected.toLocaleDateString('es-CL',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).replace(/^./,c=>c.toUpperCase());const movements=movBy[key]||[],incomes=movements.filter(r=>r.tipo==='ingreso'),expenses=movements.filter(r=>r.tipo==='gasto'),payments=futBy[key]||[];const totalIncome=incomes.reduce((s,r)=>s+Number(r.monto),0),totalExpense=expenses.reduce((s,r)=>s+Number(r.monto),0),totalPayments=payments.reduce((s,r)=>s+Number(r.monto),0);const movementRow=r=>`<div class="detail-item"><div class="detail-main"><strong>${esc(r.categoria||'Sin categoría')}</strong><span>${esc(r.descripcion||'Sin descripción')}</span></div><strong>${CLP(r.monto)}</strong></div>`;const paymentRow=r=>`<div class="detail-item scheduled-item"><div class="detail-main"><strong>${esc(r.concepto||'Sin concepto')}</strong><span>${esc(r.categoria||'Sin categoría')} · ${esc(r.periodicidad||'unico')}</span>${r.notas?`<span>${esc(r.notas)}</span>`:''}<span class="payment-state">Pendiente · vence ${esc(r.fecha_vencimiento||key)}</span></div><strong>${CLP(r.monto)}</strong></div>`;box.innerHTML=`<div class="selected-day-header"><div><span class="plan-kicker">Detalle del día seleccionado</span><h3>${dateLabel}</h3></div><span class="selected-date-pill">${key}</span></div><div class="day-detail-grid"><article class="day-detail-block income-detail"><div class="detail-title"><div><span>Ingresos registrados</span><strong>${CLP(totalIncome)}</strong></div><span class="detail-count">${incomes.length} registro${incomes.length===1?'':'s'}</span></div>${incomes.length?incomes.map(movementRow).join(''):'<p class="muted">No hay ingresos registrados para este día.</p>'}</article><article class="day-detail-block expense-detail"><div class="detail-title"><div><span>Gastos registrados</span><strong>${CLP(totalExpense)}</strong></div><span class="detail-count">${expenses.length} registro${expenses.length===1?'':'s'}</span></div>${expenses.length?expenses.map(movementRow).join(''):'<p class="muted">No hay gastos registrados para este día.</p>'}<div class="scheduled-section"><div class="scheduled-section-header"><div><span>Pagos programados / pendientes</span><strong>${CLP(totalPayments)}</strong></div><span class="detail-count">${payments.length} pendiente${payments.length===1?'':'s'}</span></div>${payments.length?payments.map(paymentRow).join(''):'<p class="muted">No hay pagos programados pendientes para este día.</p>'}</div></article></div>${row?`<div class="selected-day-balance"><span>Saldo acumulado al cierre del día</span><strong>${CLP(row.balance)}</strong></div>`:''}`}
...
function calculateIncomePlan(mov,fut){const t=today(),actualMov=mov.filter(r=>r.fecha<=t),currentBalance=actualMov.reduce((s,r)=>s+signedMovement(r),0),pending=fut.filter(r=>r.estado==='pendiente'&&r.fecha_vencimiento>=t).sort((a,b)=>a.fecha_vencimiento.localeCompare(b.fecha_vencimiento));if(!pending.length)return{has:false,currentBalance,savingsSuggested:actualMov.filter(r=>r.tipo==='ingreso').reduce((s,r)=>s+Number(r.monto)*.10,0)};const targetDate=pending[0].fecha_vencimiento,sameDay=pending.filter(r=>r.fecha_vencimiento===targetDate),paymentsTarget=sameDay.reduce((s,r)=>s+Number(r.monto),0),futureIncomesBefore=mov.filter(r=>r.tipo==='ingreso'&&r.fecha>t&&r.fecha<=targetDate).reduce((s,r)=>s+Number(r.monto),0),futureExpensesBefore=mov.filter(r=>r.tipo==='gasto'&&r.fecha>t&&r.fecha<=targetDate).reduce((s,r)=>s+Number(r.monto),0),otherPendingBefore=fut.filter(r=>r.estado==='pendiente'&&r.fecha_vencimiento>t&&r.fecha_vencimiento<targetDate).reduce((s,r)=>s+Number(r.monto),0),availableBefore=Math.max(0,currentBalance+futureIncomesBefore-futureExpensesBefore-otherPendingBefore),gap=Math.max(0,paymentsTarget-availableBefore),grossNeeded=gap/.9,days=Math.max(1,Math.ceil((parseDate(targetDate)-parseDate(t))/86400000)),daily=Math.ceil(grossNeeded/days),dailySaving=Math.ceil(daily*.1),actualIncome=actualMov.filter(r=>r.tipo==='ingreso').reduce((s,r)=>s+Number(r.monto),0),futureIncome=mov.filter(r=>r.tipo==='ingreso'&&r.fecha>t).reduce((s,r)=>s+Number(r.monto),0);return{has:true,currentBalance,targetDate,sameDayCount:sameDay.length,paymentsTarget,futureIncomesBefore,futureExpensesBefore,otherPendingBefore,availableBefore,gap,grossNeeded,days,daily,dailySaving,savingsSuggested:actualIncome*.1,futureSavingsSuggested:futureIncome*.1}}
function renderIncomePlan(mov,fut){const p=calculateIncomePlan(mov,fut),box=$('incomePlan');if(!box)return;if(!p.has){box.innerHTML=`<div class="plan-ok"><strong>Sin obligaciones futuras pendientes.</strong><span>No necesitas una meta diaria adicional por pagos programados en este momento.</span></div><div class="plan-savings"><b>Ahorro sugerido acumulado (10% de ingresos registrados):</b> ${CLP(p.savingsSuggested)}</div>`;return}const dateLabel=parseDate(p.targetDate).toLocaleDateString('es-CL',{day:'numeric',month:'long'}),paymentLabel=p.sameDayCount>1?`${p.sameDayCount} obligaciones`:'la próxima obligación',gapText=p.gap>0?`Faltan ${CLP(p.gap)} para cubrirlas.`:'El saldo disponible alcanza para cubrirlas.';box.innerHTML=`<div class="plan-header"><div><span class="plan-kicker">Sugerencia de ingreso</span><h3>${paymentLabel} · ${dateLabel}</h3></div><span class="suggest-pill">NO es una obligación</span></div><div class="plan-main"><div><span>Ingreso estimado por día</span><strong>${CLP(p.daily)}</strong><small>Durante ${p.days} día(s), considerando reservar 10% para ahorro.</small></div><div class="plan-target"><span>Responsabilidades de esa fecha</span><strong>${CLP(p.paymentsTarget)}</strong><small>${gapText}</small></div></div><div class="plan-details"><span>Saldo disponible antes: <b>${CLP(p.availableBefore)}</b></span><span>Ingreso bruto adicional estimado: <b>${CLP(p.grossNeeded)}</b></span><span>Ahorro 10% diario sugerido: <b>${CLP(p.dailySaving)}</b></span></div><div class="plan-savings"><b>Ahorro sugerido acumulado:</b> ${CLP(p.savingsSuggested)} <span>· 10% de los ingresos ya registrados</span></div><p class="plan-note">Se calcula con tu saldo actual, ingresos y gastos futuros registrados y compromisos pendientes anteriores o de la misma fecha. Si cambias un monto o fecha, la sugerencia se actualiza.</p>`}
function renderDash(mov,fut,sav){const t=today(),d=parseDate(t),prefix=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`,actual=mov.filter(r=>r.fecha<=t),future=mov.filter(r=>r.fecha>t),inc=actual.filter(r=>r.tipo==='ingreso'&&r.fecha.startsWith(prefix)).reduce((s,r)=>s+Number(r.monto),0),exp=actual.filter(r=>r.tipo==='gasto'&&r.fecha.startsWith(prefix)).reduce((s,r)=>s+Number(r.monto),0),ci=actual.filter(r=>r.tipo==='ingreso').reduce((s,r)=>s+Number(r.monto),0),ce=actual.filter(r=>r.tipo==='gasto').reduce((s,r)=>s+Number(r.monto),0),comm=fut.filter(r=>r.estado==='pendiente').reduce((s,r)=>s+Number(r.monto),0),fi=future.filter(r=>r.tipo==='ingreso').reduce((s,r)=>s+Number(r.monto),0),fe=future.filter(r=>r.tipo==='gasto').reduce((s,r)=>s+Number(r.monto),0),ah=sav.reduce((s,r)=>s+(r.tipo==='aporte'?Number(r.monto):-Number(r.monto)),0),saldo=ci-ce,proj=saldo+fi-fe-comm,pendingMonth=fut.filter(r=>r.estado==='pendiente'&&r.fecha_vencimiento.startsWith(prefix)),totalPendingMonth=pendingMonth.reduce((s,r)=>s+Number(r.monto),0),totalPaidMonth=exp,totalToPayMonth=totalPaidMonth+totalPendingMonth;$('saldo').textContent=CLP(saldo);$('ingresosMes').textContent=CLP(inc);$('gastosMes').textContent=CLP(exp);$('comprometido').textContent=CLP(comm);$('proyectado').textContent=CLP(proj);$('ahorroTotal').textContent=CLP(ah);$('ingresosFuturos').textContent=CLP(fi);$('gastosFuturos').textContent=CLP(fe);$('totalPagadoMes').textContent=CLP(totalPaidMonth);$('totalPendienteMes').textContent=CLP(totalPendingMonth);$('totalGastosPagarMes').textContent=CLP(totalToPayMonth);$('monthlySummaryLabel').textContent=`${monthLabel(d)} · actualizado al ${d.getDate()} de ${d.toLocaleDateString('es-CL',{month:'long'})}`;renderIncomePlan(mov,fut);const p=fut.filter(r=>r.estado==='pendiente').slice(0,8);$('proximosPagos').innerHTML=p.length?p.map(r=>`<div class="row"><span>${esc(r.concepto)}<br><small>${r.fecha_vencimiento}</small></span><strong>${CLP(r.monto)}</strong></div>`).join(''):'<p class="muted">No tienes pagos pendientes.</p>'}
